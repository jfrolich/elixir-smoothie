...
const readFile = filename =>
  fs.readFileSync(filename, { encoding: 'utf-8' }).replace(/^\s+|\s+$/g, '');
const snapshot = filename =>
  readFile(path.join('__tests__', 'data', 'snapshots', filename));
const build = (templateDir, filename) =>
  readFile(path.join('__tests__', 'data', templateDir, 'build', filename));
...
test('expect to fail when no template dir specified', () => {
  return runScript().then(
    ({ stderr }) => expect(stderr).toBe('Error: No template dir specified\n')
  );
});

test(
  'handle templates without layouts',
  () =>
    runScript({
...
test(
  'handle templates with foundation',
  () =>
    runScript({
      env: Object.assign(PROCESS_ENV, {
...
test(
  'handle templates with layouts and sass',
  () =>
    runScript({
      env: Object.assign(PROCESS_ENV, {
      ...
      expect(
        build('simple_with_layout_and_stylesheet', html)
      ).toEqual(snapshot(html));
      expect(
        build('simple_with_layout_and_stylesheet', txt)
      ).toEqual(snapshot(txt));
    })
);
